...
The `histoc` assembles functions for histocompatibity testing in the context of kidney transplantation.   

Package main functions allows to simulate kidney allocation systems for the distribution of deceased donors' grafts.

Currently, it is possible to simulate allocation rules implemented in Portugal (PT model), in countries within Eurtotransplant (ET model), in the United Kingdom (UK model), and a system previously suggested by Lima *et al.* (Lima's model), using functions: `pts()`, `et()`, `uk()` and `lima()`, respectively.

Each one of these functions have as inputs a data frame with transplant candidates' clinical and demographic characteristics, a data frame with candidates' HLA antibodies and inputs for a potential donor.

For all the models a virtual crossmatch between the donor and transplant candidates is performed (`xmatch()`). And, only those candidates with a negative crossmatch and ABo compatible can opt to donor's kidney.
...
A simplified version of EuroTransplant Kidney Allocation System (ETKAS) for first time kidney only candidates with more than 18 years old and not having donated one of their own kidneys can be simulated through `et()`.
...
United Kingdom (UK) deceased donor kidney allocation for transplantation is firstly based on the definition of two ranked Tiers of candidates eligible for the donor (`Tier`):
...
A color prioritization (`cp()`) of waiting list transplant candidates.

With this model, transplant candidates are classified according to their clinical urgency (red color), and regarding their time on `dialysis` and cPRA value `cp`. With an orange color are marked those patients with a `cPRA` >85% or with a time on `dialysis` higher than waiting time 3rd quartile. As yellow are classified the patients with a `cPRA` >50% or with a time on `dialysis` higher than waiting time median. And, as green are classified all the rest.

Also, patients are allocated to donors within the same age group (old to old program) (`SP`), mimicking EuroTransplant senior program (`sp()`).
